...
def call_openrouter(prompt: str, model: str, temperature: float, top_p: float, seed: int, timeout: int) -> dict:
...
def call_ollama(prompt: str, model: str, temperature: float, top_p: float, seed: int, timeout: int) -> dict:
...
def main() -> int:
    ...
    started = time.time()
    try:
        result = call_openrouter(prompt, args.primary_model, args.temperature, args.top_p, args.seed, args.timeout)
    except Exception as exc:
        errors.append({'provider': 'openrouter', 'model': args.primary_model, 'error': str(exc)})
        try:
            result = call_openrouter(prompt, args.secondary_model, max(0.88, args.temperature - 0.06), args.top_p, args.seed, args.timeout)
        except Exception as exc2:
            errors.append({'provider': 'openrouter', 'model': args.secondary_model, 'error': str(exc2)})
            try:
                result = call_ollama(prompt, args.fallback_model, max(0.75, args.temperature - 0.12), args.top_p, args.seed, args.timeout)
